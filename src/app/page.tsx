...
async function getCourseData(lang: "en" | "bn" = "en"): Promise<CourseData | null> {
  try {
    const res = await fetch(
      `https://api.10minuteschool.com/discovery-service/api/v1/products/ielts-course?lang=${lang}`,
      {
        headers: { "X-TENMS-SOURCE-PLATFORM": "web", accept: "application/json" },
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { 
    ...
    description: data?.seo?.description ?? 'Join our IELTS Course today to achieve your desired band score.'
  };
}
...
export default async function Home() {
  ...
  if (!courseData) {
    return <main className="max-w-6xl mx-auto p-8 text-center">Failed to load course data. Please try again later.</main>
  }

  const instructorSection = courseData?.sections?.find((sec) => sec.type === "instructors");
    ...
    <main className="max-w-6xl mx-auto p-4 lg:p-8 pt-10">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          {exclusiveFeaturesSection && (<CourseExclusiveFeatures section={exclusiveFeaturesSection} />)}
